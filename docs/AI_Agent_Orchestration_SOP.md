...
```bash
# 在專案根目錄執行全自動初始化並啟用自動歸檔 (✅ 推薦)
ai-brain init -a

# 在專案根目錄執行全自動初始化 (⚠️ 預設不啟用自動歸檔，需手動執行 ai-brain include 啟用)
ai-brain init
```

| 指令                    | 差異                                                                                |
| :---------------------- | :---------------------------------------------------------------------------------- |
| `ai-brain init -a`      | 全自動配置，並自動將專案加入自動歸檔白名單（含全域 23:30 自動歸檔 Cron Job）       |
| `ai-brain init`         | 全自動配置（含 Wing、.claude/CLAUDE.md、Git Hooks，但⚠️ 預設不歸檔）                 |
...
| 指令                 | 場景                           | 安全等級        |
| :------------------- | :----------------------------- | :-------------- |
| `ai-brain init -a`   | 全自動初始化 + 啟用自動歸檔     | ✅ 安全          |
| `ai-brain init`      | 全自動初始化 (預設不自動歸檔)   | ✅ 安全          |
