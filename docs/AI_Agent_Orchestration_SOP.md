...
| `INSTALLED` + 專案已有 `graphify-out/`                | 直接開始開發（參考「第二部分：日常指令」）        |
| `INSTALLED` + 專案沒有 `graphify-out/`                | 執行 `ai-brain init`（參考「第二部分：初始化」）  |

### Step 2：檢測專案是否已初始化

```bash
# 檢查專案大腦配置是否存在
[ -d "graphify-out" ] && echo "INITIALIZED" || echo "NOT_INITIALIZED"
```

| 結果                                                  | 動作                                                                |
| :---------------------------------------------------- | :------------------------------------------------------------------ |
| `NOT_INITIALIZED`                                     | 執行 `ai-brain init`（基礎配置）或 `ai-brain full-init`（含 Cron）  |
...
```bash
# 快速三合一檢查
for cmd in mempalace claude-mem graphify; do
...
| Gemini / Antigravity IDE     | 寫入 `~/.gemini/config/mcp_config.json`（`full-init` 自動處理，或手動執行 python3 寫入）        |
...
```mermaid
graph TD
    A[新專案] -->|ai-brain full-init| B(一鍵初始化大腦)
...
| 指令                    | 差異                                                                                |
| :---------------------- | :---------------------------------------------------------------------------------- |
| `ai-brain init`         | 僅專案配置（MemPalace Wing、Graphify 技能、CLAUDE.md、Git Hooks）                   |
| `ai-brain full-init`    | init 全部動作 + 註冊全域 Cron Job（每天晚上 23:30 自動記憶歸檔）                             |
...
```bash
ai-brain clean       # 清除當前專案大腦配置（mempalace.yaml, graphify-out/, .claude/, git hooks）
...
| 指令                 | 場景                           | 安全等級        |
| :------------------- | :----------------------------- | :-------------- |
| `ai-brain init`      | 新專案初始化                   | ✅ 安全          |
| `ai-brain full-init` | 新專案初始化 + Cron            | ✅ 安全          |
...
1. Cursor 會自動讀取專案根目錄下的 `CLAUDE.md` 與 `graphify-out/`，不需額外設定
...
1. **共享長期記憶機制**：
   ...
   - `ai-brain full-init` 會自動偵測 `~/.gemini/` 目錄是否存在，若存在則自動寫入 `mcp_config.json`，完成後即可與 Claude Code / OpenCode 共享同一長期記憶資料庫。
...
+─────────────────────────────────────────────────────────────+
  ...
  【1. 啟動與導航】 ───►  讀取 CLAUDE.md  ───►  加載 Graphify 代碼地圖
