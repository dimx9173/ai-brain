...
# Legacy GC cron marker — used only for cleanup during uninstall.
# GC is now idle-based (runs on first launch after 7+ days), not cron-based.
_LEGACY_GC_MARKER = "ai-brain gc"
...
def uninstall() -> bool:
    """Remove any ai-brain-owned cron lines (daily archive + legacy GC)."""
    ...
    lines = _read_crontab()
    if not any(CRON_MARKER in line or _LEGACY_GC_MARKER in line for line in lines):
        yellow(" 未偵測到 ai-brain 相關 Cron Job，無須移除。")
        return True

    remaining = [
        line for line in lines
        if CRON_MARKER not in line and _LEGACY_GC_MARKER not in line
    ]
